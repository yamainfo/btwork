...
if __name__ == "__main__":
    ...
    if args.save_checkpoints:
        # Use different monitoring for classification vs regression
        monitor_metric = "val/accuracy" if config.get('hyperparams', {}).get('loss') == 'bce' else "val/rmse"
        mode = "max" if monitor_metric == "val/accuracy" else "min"
        filename = 'epoch{epoch}-val-acc{val/accuracy:.4f}' if monitor_metric == "val/accuracy" else 'epoch{epoch}-val-rmse{val/rmse:.4f}'
