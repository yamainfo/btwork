...
def get_yfinance_ohlcv(symbol, interval, start_time, limit=1000):
    ...
    try:
        ...
        data['Datetime'] = data['Datetime'].dt.tz_localize(None).dt.tz_localize('UTC')
...
def start_download():
    end_date = datetime.now(timezone.utc) # 60 days data
...
def predict(filename):
    ...
    # Generate timestamps for next 60 minutes
    now = datetime.utcnow().replace(second=0, microsecond=0)
    df_pred["timestamp"] = [now + timedelta(minutes=i) for i in range(1, FORECAST_LENGTH+1)]
